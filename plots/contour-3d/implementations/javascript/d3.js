...
const floorCorners = [
  [xs[0], ys[0], floorZ],
  [xs[GRID_N - 1], ys[0], floorZ],
  [xs[GRID_N - 1], ys[GRID_N - 1], floorZ],
  [xs[0], ys[GRID_N - 1], floorZ],
];
...
const axisLabels = [
  { pos: [xAxisOtherEnd, anchorY + outwardYSign * 2.5, floorZ], text: "X" },
  { pos: [anchorX + outwardXSign * 2.5, yAxisOtherEnd, floorZ], text: "Y" },
  { pos: [anchorX + outwardXSign * 2.5, anchorY, zMaxScaled], text: "Value" },
];
...
// --- Floor: base topographic contour map, painted before the surface -------
svg
  .append("g")
...
// --- Surface: true 3D geometry, filled by discrete contour band -----------
const surfaceGroup = svg.append("g").attr("stroke-width", 0.6);
surfaceGroup
  .selectAll("path")
  .data(quads)
  .join("path")
...
// --- Isolines draped on the surface at their true level height -------------
svg
  .append("g")
  .attr("fill", "none")
