...
# CRITICAL: Remove the script directory from sys.path to prevent importing
# the local pygal.py file instead of the pygal library
...
chart.add("KDE Curve", kde_points, stroke_style={"width": 5})

# Add rug marks as dots at y=0 with better styling
# Each data point gets a small marker at the bottom of the plot
sorted_values = np.sort(values)
rug_points = [(float(v), 0.0) for v in sorted_values]

# Add rug marks using the second Okabe-Ito color for visual distinction
chart.add("Rug Marks", rug_points, stroke=False, show_dots=True, fill=False, dots_size=8)

# Save as PNG and HTML with theme-suffixed naming
