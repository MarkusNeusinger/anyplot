...
fig.add_trace(
    go.Bar(
        ...
        x=values_sorted,
        orientation="h",
        marker={"color": colors, "line": {"color": PAGE_BG, "width": 1}},
        text=[f"{v:+d}" for v in values_sorted],
        textposition="outside",
        textfont={"size": 12, "color": colors},
        hovertemplate="%{y}<br>Score: %{x:+d}<extra></extra>",
    )
)
...
fig.update_layout(
    autosize=False,
    title={
        "text": "bar-diverging · python · plotly · anyplot.ai",
