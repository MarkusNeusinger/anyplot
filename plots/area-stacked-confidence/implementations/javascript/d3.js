...
const data = dates.map((date, i) => {
  ...
  productLines.forEach((_, s) => {
    ...
    row[`${key}_band`] = 0.04 + (i / (nQuarters - 1)) * 0.22;
  });
  return row;
});
...
// Stacked central areas first, translucent bands on top (so a band bleeding
// past its series' own boundary blends into the neighbor, showing where the
// two series' uncertainty overlaps), then crisp boundary lines on top of both.
stacked.forEach((layer, i) => {
  g.append("path").datum(layer).attr("fill", t.palette[i]).attr("fill-opacity", 0.88).attr("d", areaCentral);
});
bands.forEach((band, i) => {
  g.append("path").datum(band).attr("fill", t.palette[i]).attr("fill-opacity", 0.3).attr("d", areaBand);
});
stacked.forEach((layer, i) => {
  g.append("path")
    .datum(layer)
...
legend
  .append("text")
  .attr("x", 0)
  .attr("y", productLines.length * 42 + 14)
  .attr("fill", t.inkSoft)
