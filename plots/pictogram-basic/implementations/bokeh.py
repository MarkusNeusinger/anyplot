...
from bokeh.models import ColumnDataSource, CustomJSTickFormatter, FixedTicker, Label, Range1d, Title
...
# Figure — use square format for vertical pictogram layout
max_icons = max(int(np.ceil(v / icon_value)) for v in values)
p = figure(
    width=3600,
    height=3600,
    x_range=Range1d(-0.7, max_icons + 0.8),
    y_range=Range1d(-0.8, n_cats - 0.3),
    toolbar_location=None,
    title="Fruit Production · pictogram-basic · bokeh · pyplots.ai",
)

# Subtitle with units context
subtitle = Title(text="Annual output in thousands of tonnes", text_font_size="22pt", text_color="#777777")
...
for i, (_cat, val) in enumerate(zip(categories, values, strict=True)):
    ...
    font_size = "24pt" if i == 0 else "20pt"
    ...
    val_label = Label(
        ...
        text_color="#333333" if i == 0 else "#666666",
