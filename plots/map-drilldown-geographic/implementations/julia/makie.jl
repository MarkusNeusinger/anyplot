...
w_vals  = Float64[   2.80,  2.10,  3.60,  1.40, 1.10,  0.70]

# Approximate geographic bounding boxes per world region (for map context)
w_boxes = [
    (-170.0, 15.0, -50.0, 80.0),   # N. America
    ( -15.0, 35.0,  45.0, 72.0),   # Europe
    (  60.0,-50.0, 180.0, 78.0),   # Asia-Pacific
    ( -85.0,-58.0, -33.0, 15.0),   # S. America
    (  25.0, 10.0,  65.0, 42.0),   # Middle East
    ( -20.0,-38.0,  55.0, 38.0),   # Africa
]

# US states — drill into North America (values sum to $2.80B = parent node)
...
s_vals = Float64[  0.60,  0.46,  0.49,  0.36,   0.27,  0.24,  0.20,   0.18]
...
# NY (-74°,43°) and MA (-71°,42°) are adjacent — stagger labels left/right
s_offsets = [(0,14),(0,14),(-14,10),(0,14),(0,14),(0,14),(14,10),(0,14)]

# Highlight the drilled-into region with a bold INK ring
w_sw = Float64[i == 1 ? 2.8 : 1.2 for i in 1:length(w_names)]
w_sc = [i == 1 ? INK : INK_SOFT for i in 1:length(w_names)]
s_sw = Float64[i == 1 ? 2.8 : 1.2 for i in 1:length(s_abbr)]
s_sc = [i == 1 ? INK : INK_SOFT for i in 1:length(s_abbr)]

box_fill   = RGBAf(INK.r, INK.g, INK.b, 0.06)
box_border = RGBAf(INK.r, INK.g, INK.b, 0.22)
...
Label(fig[0, 1:3],
    ...
    fontsize=17, color=INK, font=:bold, halign=:center, padding=(0, 0, 2, 0),
)

# Breadcrumb navigation strip
Label(fig[1, 1:3],
    "World  ▶  N. America  ▶  California",
    fontsize=11, color=INK_SOFT, halign=:center, padding=(0, 0, 0, 2),
)
...
for (xmin, ymin, xmax, ymax) in w_boxes
    poly!(ax1,
        [Point2f(xmin, ymin), Point2f(xmax, ymin), Point2f(xmax, ymax), Point2f(xmin, ymax)];
        color=box_fill, strokecolor=box_border, strokewidth=0.7,
    )
end
scatter!(ax1, w_lon, w_lat;
    color=w_norm, colormap=ANYPLOT_SEQ, colorrange=(0.0, 1.0),
    markersize=28, strokewidth=w_sw, strokecolor=w_sc,
)
for i in eachindex(w_names)
    text!(ax1, w_lon[i], w_lat[i];
        text="$(w_names[i])\n\$$(w_vals[i])B",
        align=(:center, :bottom), offset=(0, 17),
        fontsize=8, color=i == 1 ? INK : INK_SOFT,
    )
end
xlims!(ax1, -180, 185)
ylims!(ax1, -65, 82)

# Panel 2 — US states bubble map (ylabel omitted — same axis type as Panel 1)
ax2 = Axis(fig[2, 2];
...
poly!(ax2,
    [Point2f(-127, 48), Point2f(-95, 49), Point2f(-82, 46),
     Point2f(-67, 47),  Point2f(-67, 25), Point2f(-97, 26),
     Point2f(-117, 32), Point2f(-127, 34)];
    color=box_fill, strokecolor=box_border, strokewidth=0.7,
)
scatter!(ax2, s_lon, s_lat;
    color=s_norm, colormap=ANYPLOT_SEQ, colorrange=(0.0, 1.0),
    markersize=24, strokewidth=s_sw, strokecolor=s_sc,
)
for i in eachindex(s_abbr)
    text!(ax2, s_lon[i], s_lat[i];
        text=s_abbr[i],
        align=(:center, :bottom), offset=s_offsets[i],
        fontsize=9, color=i == 1 ? INK : INK_SOFT,
    )
end
xlims!(ax2, -132, -62)
ylims!(ax2, 22, 52)
...
for i in eachindex(c_vals)
    text!(ax3, c_vals[i] + 0.006, i;
        text="\$$(c_vals[i])B",
        align=(:left, :center),
