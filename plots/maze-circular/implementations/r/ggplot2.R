...
CENTER_R     <- 0.5   # radius of goal boundary (inner wall of ring 1)
ENTRY_SECTOR <- 1     # outer boundary gap for maze entry

ring_in  <- function(r) CENTER_R + (r - 1)
ring_out <- function(r) CENTER_R + r
OUTER_R  <- ring_out(RINGS)  # = 7.5

# Angle at fraction frac (0=start, 1=end) of sector s, clockwise from top
sec_angle <- function(s, frac = 0) (s - 1 + frac) * ANGLE_STEP - pi / 2

# --- Iterative DFS maze generation
# arc_open[r, s]    = passage between ring r and r+1 at sector s (r = 1..RINGS-1)
# radial_open[r, s] = passage from sector s to s+1 in ring r (wraps around)
...
# Open center passage: one gap in the inner boundary of ring 1
center_sector <- sample(seq_len(SECTORS), 1)

# --- Accumulate wall geometry
walls <- NULL
seg   <- 0L
...
# Outer boundary — full circle minus entry gap
{
  a0   <- sec_angle(ENTRY_SECTOR, 0)
  a1   <- sec_angle(ENTRY_SECTOR, 1)
  angs <- seq(a1, a0 + 2 * pi, length.out = 300)
  append_seg(OUTER_R * cos(angs), OUTER_R * sin(angs))
}

# Center boundary — full circle minus center_sector gap
{
...
p <- ggplot() +
  geom_path(
    ...
    color = INK, linewidth = 0.5, lineend = "round"
  ) +
  geom_polygon(
  ...
  labs(title = "maze-circular · r · ggplot2 · anyplot.ai") +
