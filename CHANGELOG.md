...
- **The API deploy gets the three edges the frontend deploy already had** — the candidate
  ...
  ever tags its own revision, so seeing ours at both ends proves every probe in between hit
  it. And the probes stop piping into `grep -q`, which exits at the first match and SIGPIPEs
