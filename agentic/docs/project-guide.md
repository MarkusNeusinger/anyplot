...
- **`prompts/`**: AI agent prompts for code generation, quality evaluation, and tagging
  ...
  - `library/`: Library-specific generation rules (15 files, one per library)
...
# Previews
preview_url: https://storage.googleapis.com/anyplot-images/plots/scatter-basic/python/matplotlib/plot-light.png
preview_html: null
...
```
gs://anyplot-images/
├── staging/{spec-id}/{language}/{library}/   # Temp images (during generation/review)
│   ├── plot-light.png                        # Generated by impl-generate.yml
│   ├── plot-dark.png                         # (theme pair — one render per theme)
│   └── plot-{light,dark}.html                # Optional (interactive libraries)
│
└── plots/{spec-id}/{language}/{library}/     # Production images (after merge to main)
    ├── plot-light.png                        # Promoted by impl-merge.yml
    ├── plot-dark.png
    └── plot-{light,dark}.html                # Optional (interactive libraries)
```
...
| `library/*.md` | Library-specific rules (15 files, one per library) |
...
impl-review.yml
    ...
    |
    +-- If REJECTED -> triggers impl-repair.yml (max 4 attempts)
            |-- Reads AI feedback
...
- **Partial implementations OK** - 9/15 done = fine
...
- **`ai-attempt-1`** … **`ai-attempt-4`** - Repair attempt counter
- **`ai-review-failed`** - AI review action failed or timed out
- **`not-feasible`** - Library cannot implement this specification (NOT_FEASIBLE verdict)
...
- **`changelog.d/<slug>.md`**, one fragment per PR, is what a PR writes — never a NEW bullet in
  ...
  more copy of a title already there) belongs in a fragment. **Exempt** (would drown the file): catalogue-only PRs
